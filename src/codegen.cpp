...
static Function *gen_cfun_wrapper(jl_function_t *ff, jl_value_t *jlrettype, jl_tupletype_t *argt, int64_t isref)
{
    ...
    for (size_t i=0; i < nargs; i++) {
        ...
        if (isref & (2<<i)) {
            ...
            val = llvm_type_rewrite(val, val->getType(), fargt[i+sret], true, byRefList[i], issigned);
        }
    ...
    if (isref&1) {
        ...
        if (type_is_ghost(crt)) {
            ...
            Value *v = julia_to_native(crt, jlrettype, r, jlrettype, false, false, false, false, true, 0, &ctx, NULL);
            bool issigned = jl_signed_type && jl_subtype(jlrettype, (jl_value_t*)jl_signed_type, 0);
            if (!sret) {
                builder.CreateRet(llvm_type_rewrite(v, crt, prt, false, false, issigned));
            }
            else {
                Value *sretVal = llvm_type_rewrite(v, crt, fargt_sig[0], false, false, issigned);
