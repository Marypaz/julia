...
writemime(io::IO, ::MIME"text/plain", x) = Base.Show.showlimited(io, x)
...
function writemime(io::IO, ::MIME"text/plain", v::AbstractVector)
    if isa(v, Range)
        ...
        if !isempty(v)
            println(io, ":")
            Base.Show.with_output_limit(()->Base.Show.print_matrix(io, v))
        end
    end
end

writemime(io::IO, ::MIME"text/plain", v::AbstractArray) =
    Base.Show.with_output_limit(()->showarray(io, v, header=true, repr=false))
